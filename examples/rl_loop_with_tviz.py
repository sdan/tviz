"""
RL training loop with tviz logging.

This is tinker_cookbook/recipes/rl_loop.py with tviz integration added.
...
def main(config: Config):
    ...
    for batch_idx in range(start_batch, n_train_batches):
        ...
        for group_idx, (sample_futures, prompt_tokens, question, answer) in enumerate(
            zip(batch_futures, batch_prompts, batch_rows["question"], batch_rows["answer"])
        ):
            ...
            # >>> tviz <<<
            # Add rollout group to visualization data
        ...
        # >>> tviz <<<
        # Log rollouts to tviz for visualization
    ...
    # >>> tviz <<<
    tviz_logger.close()
    logger.info(f"View training run at: {tviz_logger.get_logger_url()}")
    # >>> tviz <<<
